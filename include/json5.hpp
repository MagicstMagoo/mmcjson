...
namespace json
{
    ...
    MMCJSON_INLINE const std::wregex parser5::unicode::space_separator =
        std::wregex(LR"([\u1680\u2000-\u200A\u202F\u205F\u3000])");

    MMCJSON_INLINE const std::wregex parser5::unicode::id_start = std::wregex(
        ...
        LR"(\uDC1D\uDC20-\uDFFF]|\uD873[\uDC00-\uDEA1\uDEB0-\uDFFF]|\uD87A[\uDC00-)"
        LR"(\uDFE0]|\uD87E[\uDC00-\uDE1D])");
    MMCJSON_INLINE const std::wregex parser5::unicode::id_continue = std::wregex(
        LR"([\xAA\xB5\xBA\xC0-\xD6\xD8-\xF6\xF8-\u02C1\u02C6-\u02D1\u02E0-)"
        LR"(\u02E4\u02EC\u02EE\u0300-\u0374\u0376\u0377\u037A-)"
        LR"(\u037D\u037F\u0386\u0388-\u038A\u038C\u038E-\u03A1\u03A3-\u03F5\u03F7-)"
        LR"(\u0481\u0483-\u0487\u048A-\u052F\u0531-\u0556\u0559\u0561-\u0587\u0591-)"
    ...
    MMCJSON_INLINE bool parser5::unicode::isSpaceSeparator(u8char ch)
    {
#ifdef _MSC_VER
    ...
    MMCJSON_INLINE bool parser5::unicode::isIdStartChar(u8char ch)
    {
#ifdef _MSC_VER
        std::wstring wstr = { (wchar_t)ch, 0 };
#else
    ...
    MMCJSON_INLINE bool parser5::unicode::isIdContinueChar(u8char ch)
    {
#ifdef _MSC_VER
        std::wstring wstr = { (wchar_t)ch, 0 };
#else
    ...
    MMCJSON_INLINE bool parser5::unicode::isDigit(u8char ch)
    {
        auto str = StringFromCharCode(ch);
        return std::regex_search(str, std::regex(R"([0-9])"));
    }

    MMCJSON_INLINE bool parser5::unicode::isHexDigit(u8char ch)
    {
        auto str = StringFromCharCode(ch);
    ...
    MMCJSON_INLINE std::optional<value> parse5(const std::string& content, std::string* error = nullptr)
    {
        return parser5::parse(content, error);
    }

    MMCJSON_INLINE std::optional<value> parser5::parse(const std::string& content, std::string* error)
    {
        try {
    ...
    MMCJSON_INLINE std::optional<value> parser5::parse()
    {
        do {
            _token = lex();
